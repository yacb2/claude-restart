...
#
# MATCHER scopes the group; omitted, the group activates on every occurrence of
# the event. claude-restart's hooks want that, so neither call site passes one —
# the parameter exists so the helper matches claude-session-handoff's, where
# SessionStart must be scoped to `startup`.
#
# Registering is remove-then-append rather than plain append, so a stale
# registration is migrated rather than skipped. Removal is per-hook, so an
# unrelated hook sharing the group survives.
add_hook() {
